...
async def get_image(prompt):
    ...
    filename = "".join([letter for letter in prompt.lower().replace(
        " ", "_") if letter.isalnum() or letter == '_'])[:100]
...
async def format_parents_discord(messages):
    ...
    if messages_output[0]["role"].lower() != "system":
        messages_output = [
            {"role": "system", "content": DEFAULT_CONTEXT}] + messages_output
...
async def on_message(message: discord.Message):
    ...
    if messages[0]["role"] == "system":
        ...
        subresponses = [response[i:i+MAX_MESSAGE_LENGTH]
